<template>
  <div class="asserts-overview">
    <!-- 顶部的八个方块 -->
    <!-- <div class="asserts-overview-data">
      <div class="asserts-overview-data-item">
        <div
    ...
    <div class="asserts-overview-data">
      <div class="asserts-overview-data-left">
        <div class="asserts-overview-data-left-title">资产总额（万元）</div>
        <div class="asserts-overview-data-left-content">
          {{ Number(assertsOverviewData.total).toLocaleString() }}
        </div>
        <img
          src="@/assets/images/img/asserts.png"
          alt=""
          class="asserts-overview-data-left-img"
        />
      </div>
      <div class="asserts-overview-data-right">
        <div
          class="asserts-overview-data-right-item"
          v-for="(item, index) in assertsOverviewData.overView"
          :key="index"
        >
          <div
            :class="[
              'asserts-overview-data-right-item-left',
              item.typeName.includes('房产')
                ? 'asserts-overview-data-right-item-house'
                : item.typeName.includes('车辆')
                ? 'asserts-overview-data-right-item-car'
                : item.typeName.includes('土地')
                ? 'asserts-overview-data-right-item-land'
                : 'asserts-overview-data-right-item-others',
            ]"
          >
            <div class="asserts-overview-data-right-item-left-icon">
              <icon-svg :icon-class="`${item.icon}`"></icon-svg>
            </div>
          </div>
          <div class="asserts-overview-data-right-item-right">
            <div class="asserts-overview-data-right-item-right-title">
              {{ Number(item.num).toLocaleString() }}
            </div>
            <div class="asserts-overview-data-right-item-right-content">
              {{ item.typeName }}
            </div>
          </div>
        </div>
        ...
        <div class="wrapper-box">
          ...
          <div class="content-top">
            ...
            <div class="top-item-right">
              ...
              <a-popover
                placement="bottom"
                overlayClassName="asserts-overview-popover"
              >
...
import {
  ...
  getAssertsOveview,
  getAssertsTableOne,
} from "@/shared/newApi/assertsAnalysis/assertsOverview.js";
import { adjustFontSize } from "@/utils/echarts.js";
import { Checkbox, Popover, Progress, Tooltip, Select } from "ant-design-vue";
import { mapState } from "vuex";
...
export default {
  ...
  methods: {
    ...
    async getAssertsOveview() {
      const json = {
        blockNames: null, //这边不需要传，但为了保持和板块的一致性，sql让传递一下
      };
    ...
    async initEcharts2() {
      ...
      this.myChart2.on("click", (params) => {
        ...
        if (
          ...
          this.currentUnitName = "全部";
          this.currentUnitCode = null;
          console.log("走1", this.currentUnitName, this.currentUnitCode);
          this.initEcharts3();
        } else {
          this.currentUnitName =
            params.treePathInfo[params.treePathInfo.length - 1].name;
          this.currentUnitCode = params.data.code;
          console.log("走2", this.currentUnitName, this.currentUnitCode);
          this.initEcharts3();
        }
      });
    },
    ...
    async getAssertsChartFour() {
      const json = {
        year: this.blockChoosedYear, //年份
        blockNameIn:
          this.choosedSegment === null ? null : [this.choosedSegment],
