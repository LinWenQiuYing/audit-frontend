  ...
  methods: {
    ...
    onCxtClick(evt) {
      ...
      if (
        ...
        target.select();
        this.handleChangeShowMenu(false); // 关闭右键菜单
        this.$emit("selectedElesChanged", [], [target.data()]);
        return false;
      ...
      const menuData = {
        position: {
          x: evt.renderedPosition.x + 245,
          y: evt.renderedPosition.y + 155,
        },
      ...
      this.handleChangeShowMenu(true);
    },
    handleKeyDown(e) {
      e.stopPropagation();
    ...
    async fetchNeighbors(currentGraph, target, currentGraphNode, json) {
      ...
      if (
        ...
        neighborsData.vertices.length > 0
      ) {
        let parentData = {};
        ...
        let elements = [...neighborsData.edges, ...neighborsData.vertices];
        const data = {
          replace: false,
          elements,
          referenceNodes: [...neighborsData.vertices],
          anchorNodeIds: json.vertexIDs,
          type: "NEIGHBOR",
        };

        const res = checkThreshold(data, currentGraph, "1");
        const { nodes } = res;
    ...
    async directedAnalysis(graph, target, currentGraphNode) {
      ...
      if (
        ...
        const res = checkThreshold(data, graph, "1", true);
