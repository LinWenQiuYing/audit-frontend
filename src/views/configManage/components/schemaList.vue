...
import { querySchemaList } from "@/shared/api/graphsStructure.js";
...
export default {
  ...
  methods: {
    async getSchemaList(searchValue) {
      ...
      const { data } = await querySchemaList(json);
      if (!data) {
      ...
      data.dateList.map((item) => {
        listData.push({
          id: item.id,
          name: item.graphChName || item.graphName,
