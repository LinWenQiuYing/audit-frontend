  ...
  <div
    ...
    :class="[
      'model-rules-content-main',
      {
        ...
        'no-after':
          !Array.isArray(ruleList.slice(-1)[0]) || ruleList.slice(-1)[0].length < 2,
      },
    ]"
  >
  ...
  methods: {
    ...
    listChange(list, value, isParam) {
      ...
      if (isParam) {
        ...
        parentArr.splice(value.slice(-1)[0], 1, list);
      } else {
        if (value.length > 1) {
          value.map((num, index) => {
          ...
          parentArr[value.slice(-2)[0]].splice(value.slice(-1)[0], 1, list);
        } else {
    ...
    copyRule(key) {
      ...
      if (key.length > 1) {
        ...
        if (key.slice(-1)[0] === parentArr.slice(-2)[0].length - 1) {
          // 复制规则组
          target = cloneDeep(parentArr[key.slice(-2)[0]]);
          parentArr.splice(key.slice(-2)[0], 0, target);
        } else {
          // 复制单个规则
          target = cloneDeep(parentArr[key.slice(-2)[0]][key.slice(-1)[0]]);
          if (!Array.isArray(target)) {
            target.id = nanoid();
          }
          parentArr[key.slice(-2)[0]].splice(key.slice(-1)[0], 0, target);
        }
      } else {
