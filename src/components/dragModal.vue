      ...
      if (visible) {
        setTimeout(() => {
          ...
          this.modalContent.onmouseup = (e) => {
            console.log("e", e, e.offsetX, e.offsetY);
            if (this.ratio) {
              const onresize =
              ...
              // 下面的48为padding区域的宽高，55为弹窗头部的高度
              if (onresize === 0) {
                // 整个所见的弹窗区域
                const ratio = (e.offsetX - 48) / (e.offsetY - 55 - 48);
                console.log("ratio", ratio);
                if (ratio >= this.ratio) {
                  this.$refs.antdBody.style.height = e.offsetY - 55 + "px";
                  this.$refs.antdBody.style.width =
                    (e.offsetY - 55) * this.ratio + "px";
                } else {
                  this.$refs.antdBody.style.width = e.offsetX + "px";
                  this.$refs.antdBody.style.height =
                    e.offsetX / this.ratio + "px";
                }
                ...
                // 弹窗body区域
                const ratio = (e.offsetX - 48) / (e.offsetY - 48);
                console.log("ratio", ratio);
                if (ratio >= this.ratio) {
                  this.$refs.antdBody.style.height = e.offsetY + "px";
                  this.$refs.antdBody.style.width =
                    e.offsetY * this.ratio + "px";
                } else {
                  this.$refs.antdBody.style.width = e.offsetX + "px";
                  this.$refs.antdBody.style.height =
                    e.offsetX / this.ratio + "px";
