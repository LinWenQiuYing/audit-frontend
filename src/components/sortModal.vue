...
export default {
  ...
  mounted() {
    console.log("排序反选", this.firstSortName);
    this.getSelected();
  },
  methods: {
    //拿到已勾选
    getSelected() {
      ...
      let isSelected = [];
      if (this.firstSortName.includes("+")) {
        const firstSortArr = this.firstSortName.split("+");
        isSelected = this.sortOptions.filter((item) =>
          firstSortArr.includes(item.label)
        );
      } else if (this.firstSortName) {
        isSelected = this.sortOptions.filter(
          (item) => item.label === this.firstSortName
        );
      } else {
        isSelected = [];
      }
      ...
      const matchingObjects = this.sortOptions.filter((option) =>
        this.sortValue.includes(option.value)
      );
      console.log("Matching objects:", matchingObjects);
      this.dragList = matchingObjects.sort((a, b) => a.rank - b.rank);
      //初次才触发
      if (this.isFirstConfirmFlag) {
        this.sortConfirm();
      }
      //反选右侧选项的顺序
    ...
    sortConfirm() {
      ...
      let fields = [];
      arr.map((item) => fields.push(item.field));
      const sortOrder = this.choosedSort === 1 ? "asc" : "desc";

      const sortString = fields
        .map((field) => `${field} ${sortOrder}`)
        .join(", ");
      this.$emit("onSortConfirm", sortString);
    },
